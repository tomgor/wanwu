                  ...
                  <el-table-column fixed="right" label="操作" width="100">
                    <template slot-scope="scope">
                      <i
                        ...
                        @click="preDelParams(scope.$index, 'body')"
                      ></i>
                    </template>
                  </el-table-column>
                </el-table>
              ...
              <el-tab-pane label="Response" name="Response">
                <div class="editable--input" v-show="mockResDataStr">
                 <codeEditor
                        ...
                        :value="checkJsonCode(JSON.stringify(mockResData))"
                        :language="'json'"
                        :readOnly="true"
                        :theme="'vs'"
                ></codeEditor>
...
export default {
  ...
  created() {
    ...
    let headersTable = [{ key: "", value: "" }]
    for(let o in headers){
      headersTable.unshift({key:o,value:headers[o]})
    }
    this.headersTable = headersTable
    console.log(this.headersTable,119)
  ...
  methods: {
    ...
    setArray(data){
      ...
      list_schema = {
        type:"object",
        object_schema:__obj_schmea,
        children:__obj_schmea,
        name: "",
        list_schema: null
      }
      return list_schema
    ...
    parseResData() {
      ...
      for (let key in obj) {
        let _type = Array.isArray(obj[key])? 'array' :  typeof obj[key]
        let object_schema = null
        let list_schema = null
        if(_type == 'array'){
           switch(typeof(obj[key][0])){
            case "object":
                list_schema = this.setArray(obj[key][0])
                break
            case 'string':
                list_schema = {
                  type:"string",
          ...
          for(let i in obj[key]){
            if(typeof(i) == 'object'){
              if(Array.isArray(obj[key][i])){
                object_schema.push({
                  name:i,
                  list_schema: this.setArray(obj[key][i]),
                  type:'array'
                })
              }else{
                let __obj_schmea = []
        ...
        this.resParamsTable.push({
          ...
          children: _type == 'array' ? [ list_schema] : object_schema ,
          type: _type,
        });
      }

      console.log(this.resParamsTable,1223, this.mockResData, this.mockResData)
