  ...
  <div>
    <div class="table-wrap list-common wrap-fullheight">
      <div class="table-box">
        ...
        />
        <el-button size="mini" type="primary" @click="preInsert" icon="el-icon-plus" class="add-btn">
        ...
        >
          ...
          <el-table-column prop="clientSecret" label="Client Secret" align="left"/>
          <el-table-column prop="redirectUri" label="Redirect Uri" align="left"/>
          ...
          <el-table-column align="left" :label="$t('common.table.operation')" width="180">
            <template slot-scope="scope">
    ...
    >
      <el-form :model="form" :rules="rules" ref="form" style="margin-top: -16px">
        <el-form-item :label="$t('oauth.name')" prop="name">
          <el-input v-model="form.name" :placeholder="$t('common.input.placeholder')" clearable/>
        </el-form-item>
        <el-form-item :label="$t('oauth.desc')" prop="desc">
          <el-input type="textarea" :rows="3" v-model="form.desc" :placeholder="$t('common.input.placeholder')"
                    clearable/>
        </el-form-item>
        <el-form-item label="Redirect Uri" prop="redirectUri">
          <el-input v-model="form.redirectUri" :placeholder="$t('common.input.placeholder')" clearable/>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="handleClose">{{ $t('common.button.cancel') }}</el-button>
        <el-button size="small" type="primary" :loading="submitLoading" @click="handleSubmit">
...
import {fetchOAuthList, createOAuth, editOAuth, deleteOAuth, changeOAuthStatus} from "@/api/permission/oauth"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        desc: [
          {max: 200, message: this.$t('common.hint.remarkLimit'), trigger: 'blur'}
        ],
        redirectUri: [
          {required: true, message: this.$t('common.input.placeholder'), trigger: 'blur'}
        ]
  ...
  methods: {
    async getTableData() {
      const searchInfo = {...this.$refs.searchInput.value && {name: this.$refs.searchInput.value}}
      this.loading = true
      try {
    ...
    preDel(row) {
      this.$confirm(this.$t('oauth.deleteHint'), this.$t('common.confirm.title'), {
