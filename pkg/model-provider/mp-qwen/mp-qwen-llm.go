...
func chatCompletionsUnary(ctx context.Context, req mp_common.ILLMReq, url string, headers ...mp_common.Header) (mp_common.ILLMResp, error) {
	if req.Stream() {
		return nil, fmt.Errorf("request %v qwen chat completions unary but stream", url)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("request %v qwen chat completions unary err: %v", url, err)
	} else if resp.StatusCode() >= 300 {
		return nil, fmt.Errorf("request %v qwen chat completions unary http status %v msg: %v", url, resp.StatusCode(), resp.String())
	}
	b, err := io.ReadAll(resp.RawResponse.Body)
	if err != nil {
		return nil, fmt.Errorf("request %v qwen chat completions unary read response body err: %v", url, err)
	}
...
func chatCompletionsStream(ctx context.Context, req mp_common.ILLMReq, url string, headers ...mp_common.Header) (<-chan mp_common.ILLMResp, error) {
	if !req.Stream() {
		return nil, fmt.Errorf("request %v qwen chat completions stream but unary", url)
	}
	...
	go func() {
		...
		resp, err := request.Post(url)
		if err != nil {
			log.Errorf("request %v qwen chat completions stream err: %v", url, err)
			return
		} else if resp.StatusCode() >= 300 {
			log.Errorf("request %v qwen chat completions stream http status %v msg: %v", url, resp.StatusCode(), resp.String())
			return
		}
