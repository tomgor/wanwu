...
from es_rag_server import app  # 绝对导入


def main():
    parser = argparse.ArgumentParser(description='Flask应用入口点')
    parser.add_argument('--port', type=int, default=5000, help='服务端口号')
    parser.add_argument('--host', type=str, default='0.0.0.0', help='服务主机')
    parser.add_argument('--workers', type=int, default=4, help='启动worker数量')
    parser.add_argument('--timeout', type=int, default=600, help='超时时间（ms）')
