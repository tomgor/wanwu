...
Final Answer: the final answer to the all original input question. Avoid using Markdown formatting in the final answer
            ...
            if self.action_type == "yuanjing_function_call":
                    ...
                    if response.status_code == 200 :
                        ...
                        if res["data"]["choices"][0]["finish_reason"] == "tool_calls":
                            ...
                            if action.upper() != "NONE":
                                ...
                                if "<think>" not in total_result and action:
                                    total_result += "<think>"
                                    if t == 1:
                                        yield [Message(role=ASSISTANT, content=f"\n--start--\n")]
                                    yield [Message(role=ASSISTANT, content=f"<think>工具名：{action}")]
                                    yield [Message(role=ASSISTANT, content=f"\n\n```请求参数：\n{action_input}\n```\n\n")]
                                    
                                elif "<think>" in total_result and action:
                                    if t == 1:
                                        yield [Message(role=ASSISTANT, content=f"\n--start--\n")]
                                ...
                                if action_input and (not any(v is None  or  (v and v.strip() == "") or (not v) for v in action_input.values())) and  all(element in keys_identify for element in matches):
                                    ...
                                    yield [Message(role=ASSISTANT, content=f"</think>\n")]
                                else:
                                    prompt = f'''
                ...
                while num_llm_calls_available > 0:
                    ...
                    for output in output_stream:
                        if output and output[-1].content:
                            ...
                            if isinstance(res,bytes):
                                res=res.decode()
                            response += res
                            if "<THINK>" in response.upper():
                                # logger.info(f"-----deepseek截断提取：stop_token效果------")
                                response_new = re.sub('<THINK>.*?</THINK>', '', response, flags=re.DOTALL)
                                if "</THINK>" in response and ("ACTION INPUT" in response_new.upper() and "}" in response_new.upper()):
                                    break
                            else:
                    ...
                    if ("Action:" in response or "Action" in response ) and action_results['func_name']:
                        ...
                        elif  self.action_type == "qwen_agent" and action_results['action_code'] == 2 :
                    ...
                    ###4）无法回答时，code返回1
                        ...
                        if t == 1:
                            ...
                            result = {"action_code": 0,"description":action_results['description'],"func_names":["yuanjing"],"func_params":func_params,"thought_inference":thought_inference,"qa_type":qa_type,"action_output":f""}
                            result = json.dumps(result, ensure_ascii=False)
                            logger.info(f"action最终的输出结果为：{result}")
                            yield [Message(role=ASSISTANT, content=result)]
                            break 
                        ...
                        if "Final Answer" in can_answer_text or "Final Answer:" in can_answer_text:
                            ###剔除think的思考过程
                            can_answer_text = re.sub('<think>.*?</think>', '', can_answer_text, flags=re.DOTALL)
                    ...
                    if "<think>" not in total_result and action:
                        ...
                        if t == 1:
                            yield [Message(role=ASSISTANT, content=f"\n--start--\n")]
                        yield [Message(role=ASSISTANT, content=f"<think>工具名：{action[0]}")]
                    elif "<think>" in total_result and action:
                        if t == 1:
                            yield [Message(role=ASSISTANT, content=f"\n--start--\n")]
                    ...
                    if action_results['action_code'] == 0:
                        ...
                        if not action:
                            ...
                            action_output = final_result if qa_type == 20 else  f"已基于{action}完成参数提取"
                            ...
                            if qa_type == 20:
                                ...
                                if is_use_api_output and  api_stream :
                                    ...
                                    output_result = json.loads(observation['description'])
                                    ...
                                    observation = json.loads(observation)
                                    plugin_result = json.loads(observation["description"])
                                    ...
                                    if observation['action_code'] == 1 :
                                        ...
                                        yield [Message(role=ASSISTANT, content=f"</think>\n")]
                                    elif observation['action_code'] == 0 and all(value is None for value in plugin_result.values()):
                                        action_code = 1
                                ...
                                if is_use_api_output or ( not is_use_api_output and action_code == 1):
                                    ...
                                    if str(action_input).startswith('```'):
                                        ...
                                        action_input = '\n' + action_input
                                    action_result = thought + f"\nAction: {action}\nAction Input: {action_input}" + observation +"Based on the above information, think about what to do next. "
                                    thought_inference += action_result
                                    text_messages[-1].content += action_result
                                    continue
...
def find_tools(text: str,num: int) -> Tuple[bool, str, str, str]:
    ...
    if  special_args_token not in text and special_obs_token not in text:
        ...
        if special_func_token not in text:
            action_result = {"func_name": action_names, "func_params": '', "action_code": 0, "final_result": '',
                             "description": ''}
            ...
            if ((not func_name) or func_name.upper() in Invalid_character ) and special_final_token.replace("\n","") not in text:
                ...
                try:
                    if func_name and ((not func_args) or func_args in Invalid_character):
                        ...
                        # func_args = func_args.replace("'", '"').replace('Observation "', '"').replace('Observation:"', '"')
                        # if "Action:" in func_args:
                        #     first_action_index = func_args.find('Action:')
                        #     func_args = func_args[:first_action_index].strip()
