...
from utils.unicom_llm import req_unicom_llm_stream_async,req_unicom_llm_nonstream_async,req_unicom_llm
...
def _convert_delta_to_message_chunk(
    ...
    role = _dict["choices"][0]["message"]["role"]
    content = _dict["choices"][0]["message"]["content"]
    additional_kwargs: Dict = {}
...
def _convert_chunk_to_generation_chunk(
    ...
    # print("in _convert_chunk_to_generation_chunk")
    # print(type(chunk))
    # print(chunk)
...
class ChatUnicomAI(BaseChatModel):
    ...
    model_name: str = Field(default="unicom-70b-chat", alias="model")
    """Model name to use."""
    ...
    openai_api_key: Optional[SecretStr] = Field(
        alias="api_key", default_factory=secret_from_env("OPENAI_API_KEY", default="123")
    )
    ...
    def _create_chat_result(
        ...
        response_dict = (
            response if isinstance(response, dict) else json.loads(response.text)
        )
        ...
        token_usage = response_dict["data"].get("usage", {})
        for res in response_dict["data"]["choices"]:
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        if "response_format" in payload and is_basemodel_subclass(
            ...
            return
        if self.include_response_headers:
            raw_response = req_unicom_llm_stream_async(payload)
            response = raw_response.parse()
            base_generation_info = {"headers": dict(raw_response.headers)}
        else:
            # print(payload)
...
from langchain_core.messages import  messages_from_dict,convert_to_messages,convert_to_openai_messages,messages_to_dict
...
if __name__ == "__main__":
    ...
    messages =  [{'role': 'user', 'content': '\n        【任务描述】\n        请根据用户输入的上下文回答问题，并遵守回答要求。\n\n        【背景知识】\n        数科生猪养殖AI服务说明书  一、功能描述  1、猪计数AI服务  该算法的输入为布置在猪圈上方的鱼眼相机定时抓拍的猪圈的图片，图片的分辨率为1280*1280及以上。为精确定位各猪圈的边界，用户可实现在相机的画面上用拉索工具标注出猪圈的边界。算法的输出为画面中猪圈区域内猪的准确数目，每头猪的轮廓的最小包围矩形框及其编号。  2、猪称重AI服务  该算法的输入为死猪侧躺在地上的图片。其中，猪身上需要放置标准尺寸的矩形纸片（建议尺寸216mm*180mm），图片的分辨率需为1280*720及以上。算法的输出为该猪的重量（以g为单位），以及画面中猪的轮廓的最小包围框。  二、性能数据  1、采用约500K的图片进行测试，猪计数和猪估重需要的请求时间分别为770ms和280ms。  2、按次付费模式：每个接口默认支持1个QPS；按路包年模式：每个接口限制1个QPS。\n\n        【回答要求】\n        - 你需要严格根据背景知识的内容回答，禁止根据常识和已知信息回答问题。\n        - 答案要完整详细。\n        - 对于不知道的信息，直接回答“未找到相关答案”\n        -----------\n        【用户问题】都有哪些服务\n        '}, {'role': 'assistant', 'content': '数科生猪养殖AI服务提供了两种主要的服务：\n\n1. **猪计数AI服务**：\n   - 输入：此服务接收的是布置在猪圈上方的鱼眼相机定时抓拍的猪圈图片，图片分辨率要求为1280*1280或以上。\n   - 功能：用户可以在相机画面上使用拉索工具来标注猪圈的边界，以便更精确地定位。算法会输出画面中猪圈区域内猪的准确数量，同时提供每头猪的轮廓的最小包围矩形框及其编号。\n\n2. **猪称重AI服务**：\n   - 输入：该服务的输入是死猪侧躺在地上的图片，且猪身上需要放置一个标准尺寸的矩形纸片（建议尺寸为216mm*180mm）。图片分辨率同样需达到1280*720或以上。\n   - 功能：算法能够估算并输出该猪的重量（以克为单位），同时给出画面中猪的轮廓的最小包围框。\n\n这两种服务都旨在通过AI技术提高生猪养殖过程中的效率和准确性。'}]
    
    query = '都是这么收费的？'

    # messages  = [{'type': 'system', 'data': {'content': "you're a good assistant, you always respond with a joke.", 'additional_kwargs': {}, 'response_metadata': {}, 'type': 'system', 'name': None, 'id': None}}, {'type': 'human', 'data': {'content': "i wonder why it's called langchain", 'additional_kwargs': {}, 'response_metadata': {}, 'type': 'human', 'name': None, 'id': None, 'example': False}}, {'type': 'ai', 'data': {'content': 'Well, I guess they thought "WordRope" and "SentenceString" just didn\'t have the same ring to it!', 'additional_kwargs': {}, 'response_metadata': {}, 'type': 'ai', 'name': None, 'id': None, 'example': False, 'tool_calls': [], 'invalid_tool_calls': [], 'usage_metadata': None}}, {'type': 'human', 'data': {'content': 'and who is harrison chasing anyways', 'additional_kwargs': {}, 'response_metadata': {}, 'type': 'human', 'name': None, 'id': None, 'example': False}}, {'type': 'ai', 'data': {'content': "Hmmm let me think.\n\nWhy, he's probably chasing after the last cup of coffee in the office!", 'additional_kwargs': {}, 'response_metadata': {}, 'type': 'ai', 'name': None, 'id': None, 'example': False, 'tool_calls': [], 'invalid_tool_calls': [], 'usage_metadata': None}}, {'type': 'human', 'data': {'content': 'what do you call a speechless parrot', 'additional_kwargs': {}, 'response_metadata': {}, 'type': 'human', 'name': None, 'id': None, 'example': False}}]
    ...
    # print(convert_to_openai_messages(lc_messages))
    print(convert_to_openai_messages(messages_from_openai(messages)))

    
    
    # model = ChatUnicomAI()
    # async def run_async_generator():
    #     async for chunk in model._astream(messages=messages, stream=True):
    #         print(chunk) 
    #         # pass
    # # model._agenerate(messages=messages)
    # # asyncio.run(model._agenerate(messages=messages))
    ...
    # result = model.with_structured_output(messages)
    # print(result)
