...
def build_bing_query(query,model_name=MODEL_NAME):
    # 获取当前UTC时间
    now_utc = datetime.datetime.now(pytz.utc)

    # 将UTC时间转换为东八区时间
    timezone_east_8 = pytz.timezone("Asia/Shanghai")
    now = now_utc.astimezone(timezone_east_8)   
    cur_date = now.strftime(f"%Y年%m月%d日")
    cur_date2 = now.strftime(f"%m月%d日")
    cur_year = now.strftime(f"%Y年")

    logger.info(f"-------build_bing_query------")
    sub_query_dict = req_llm_structed_output(
    ...
    sub_query_list = sub_query_dict[0].get("query_list", [])
    if sub_query_list:
...
def handle_sub_query(
    ...
    hybrid_search_list = hybrid_clean_search(sub_query, result_len=result_len)  
    ...
    if is_rerank:        
        # 使用新的重排序函数
        sub_aggregated_results = hybrid_rerank_results(sub_query, sub_results, top_k, threshold)
        
        if sub_aggregated_results:
            return sub_aggregated_results[:top_k], hybrid_search_list
        else:
            # 如果重排序结果为空，使用搜索引擎原始结果
            return hybrid_search_list[:top_k], hybrid_search_list
    else:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        try:
            if bing_weight==1.0:
                ...
                bocha_search_task = executor.submit(bocha_cleaned_search, sub_query,result_len = result_len) 
...
def get_bing_multi_search_result(query,bing_top_k,bing_time_out,bing_target_success,bing_result_len,model,days_limit=-1,auto_citation=False):
    prompt =''
    aggregated_results = []
    ...
    # logger.info(f"BING  query rewirte time: {time_difference1}")
    # print(f"BING  query rewirte time: {time_difference1}")
    logger.info(f"BING sub query: {json.dumps(sub_query_list,ensure_ascii=False)}")
    # print(f"BING sub query: {json.dumps(sub_query_list,ensure_ascii=False)}")
    ...
    if len(sub_query_list)>1:
        aggregated_results,_ = req_bing_multiquery(sub_query_list,time_out=time_out,top_k= math.ceil(BING_TOP_K*top_k_ration),days_limit=days_limit,result_len = result_len,target_success = bing_target_success)
        finish_time2 = datetime.datetime.now()
    
    else:
        aggregated_results,_ = handle_sub_query(sub_query_list[0],bing_target_success,bing_top_k,BING_THRESHOLD,BING_SENTENCE_SIZE,bing_time_out,days_limit,bing_result_len)
    ...
    # logger.info(f"bing_multi_search_result: {json.dumps(aggregated_results,ensure_ascii=False)}")
    if auto_citation:        
        combined_list = combine_search_list(aggregated_results)
        logger.info(f"合并对照: {len(aggregated_results)} -->{len(combined_list)}")
    else:
    ...
    # print(f"BING multi_query time: {time_difference2 + time_difference1}")
    
    # logger.info(f"bing_multi_search_result: {json.dumps(combined_list,ensure_ascii=False)}")
    
    
    #----准备提示词
    prompt = build_bing_prompt_from_search_list(query,combined_list,auto_citation,model)    
    ...
    # print(f"bing_multi_search_result: {time_difference2 + time_difference1}")

    # print(prompt)
    # logger.info(f"BING prompt: {prompt}")
    return prompt,combined_list
async def async_search(query, bing_top_k,bing_time_out,bing_target_success,bing_result_len,model,days_limit=-1, auto_citation=False):
...
# 启动任务并返回任务对象
def start_async_search(loop, query,bing_top_k,bing_time_out,bing_target_success,bing_result_len,model,days_limit, auto_citation):
    task = loop.create_task(async_search(query, bing_top_k,bing_time_out,bing_target_success,bing_result_len,model,days_limit, auto_citation))
